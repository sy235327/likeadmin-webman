        ...
        <popup ref="popupRef" :title="popupTitle" :async="true" width="550px" @confirm="handleSubmit" @close="handleClose">
            <el-form ref="formRef" :model="formData" label-width="80px" :rules="formRules">
                ...
                <el-form-item v-if="formData.type == MenuEnum.MENU" label="组件路径" prop="component">
                    <div class="flex-1">
                        <el-autocomplete class="w-full" v-model="formData.component" :fetch-suggestions="querySearch" clearable placeholder="请输入组件路径" />
                ...
                <el-form-item v-if="formData.type != MenuEnum.CATALOGUE" label="权限字符" prop="perms">
                    <div class="flex-1">
                ...
                <el-form-item v-if="formData.type != MenuEnum.BUTTON" label="是否显示" prop="is_show" required>
                    <div>
                ...
                <el-form-item v-if="formData.type != MenuEnum.BUTTON" label="菜单状态" prop="is_disable" required>
                    <div>
...
const querySearch = (queryString: string, cb: any) => {
    const results = queryString ? componentsOptions.value.filter((item) => item.toLowerCase().includes(queryString.toLowerCase())) : componentsOptions.value
...
const getMenu = async () => {
    ...
    menu.children = arrayToTree(treeToArray(data.lists).filter((item) => item.type != MenuEnum.BUTTON))
