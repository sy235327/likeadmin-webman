...
class WeChatConfigService
{
    ...
    public static function getPayConfigByTerminal($terminal): array
    {
        ...
        if (!file_exists(root_path() . 'runtime/cert')) {
            mkdir(root_path() . 'runtime/cert', 0775, true);
        }
        //写入文件
        $apiclientCert = $pay['config']['apiclient_cert'] ?? '';
        $apiclientKey = $pay['config']['apiclient_key'] ?? '';

        $certPath = root_path() . 'runtime/cert/' . md5($apiclientCert) . '.pem';
        $keyPath = root_path() . 'runtime/cert/' . md5($apiclientKey) . '.pem';
