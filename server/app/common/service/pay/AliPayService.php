...
//        $options->gatewayHost = 'openapi.alipaydev.com'; //测试沙箱地址
        ...
        if ($config['config']['mode'] == 'certificate') {
            ...
            if (!file_exists(root_path() . 'runtime/certificate')) {
                mkdir(root_path() . 'runtime/certificate', 0775, true);
            }
            //写入文件
            $publicCert = $config['config']['public_cert'] ?? '';
            $aliPublicCert = $config['config']['ali_public_cert'] ?? '';
            $aliRootCert = $config['config']['ali_root_cert'] ?? '';
            $publicCertPath = root_path() . 'runtime/certificate/' . md5($publicCert) . '.crt';
            $aliPublicCertPath = root_path() . 'runtime/certificate/' . md5($aliPublicCert) . '.crt';
            $aliRootCertPath = root_path() . 'runtime/certificate/' . md5($aliRootCert) . '.crt';
